...
#    traitInfo_obs_zlog[traitInfo_obs_sparse] <- NA
    ...
    if(resdo|!file.exists(file.path(origin,"_2021","data","analyses","Point_wise",
                                    RepNum,t_choice,ObsOrTD,Percent,paste0("res_2021.csv")))){
  ...
  for(t in 1:ncol(traitInfo_obs_zlog)){
    ...
    for(i in 1: nrow(traitInfo_obs_zlog)){
      ...
      if(sum(ix1)>1){
        res$dist_spec_obs_zlog_sparse[ix_trait[i]] <- mean(as.matrix(
          dist(c(traitInfo_obs_zlog_sparse[i,t],
                 traitInfo_obs_zlog_sparse[ix,t])))[,1],na.rm = TRUE)
      ...
      if(sum(ix1)>1){
        res$dist_gen_obs_zlog_sparse[ix_trait[i]] <- mean(as.matrix(
          dist(c(traitInfo_obs_zlog_sparse[i,t],
                 traitInfo_obs_zlog_sparse[ix,t])))[,1],na.rm = TRUE)
