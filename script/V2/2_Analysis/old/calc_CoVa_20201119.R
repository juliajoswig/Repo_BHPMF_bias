        ...
        TD_choice = TD_choices[wtd]
         ...
         for(GapPercent in gappercents){
          ...
          if((file.exists(dat_path)&file.exists(dat_path_org1)&file.exists(trait_path))|
            ...
            save(gap_ix,file=file.path(origin,"runs",paste0("Rep",RepNum), 
                                paste0("p",GapPercent1,"_",trait_sub),
                                TD_choice,"data", "gap_ix2020.csv")) 
            # create list where everything gets written into
            out_list <- list()
