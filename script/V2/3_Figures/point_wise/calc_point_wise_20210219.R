...
for(GapPercent in gappercents){
  ...
  if(TD_choice=="rainfor"){taxInfo <- taxInfo=out$rainforTD_tax}
  if(TD_choice=="guido"){taxInfo <- taxInfo=out$guidoTD_tax}
...
write.csv(res,file=file.path(origin,"_2021","data","analyses","Point_wise",paste0("res",TD_choice,".csv")))
res<- read.csv(file=file.path(origin,"_2021","data","analyses","Point_wise",paste0("res",TD_choice,".csv")))
