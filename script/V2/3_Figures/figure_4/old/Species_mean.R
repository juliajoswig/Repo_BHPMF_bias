...
ObsOrTD="Obs_obs"
Percent=80
resENV <- read.csv(file=file.path(origin,"_2021","data","analyses","Point_wise",RepNum,t_choice,ObsOrTD,Percent,paste0("res_2021_08.csv")))
...
{
  if(t_choice=="data"){
  pdf(file=file.path(origin,"_2021","figures","Figure_6","Figure_6_Spider.pdf"),width = 8,height=11)
  }
  if(t_choice=="data_2"){
    pdf(file=file.path(origin,"_2021","figures","Figure_6","Figure_6_SpiderTD2.pdf"),width = 8,height=11)}
  ...
  t=1
  for(t in 1:length(trait_names)){
    data_now <- c(apply(abs(group_TD[[t]][,seq(1,12,2)]),MARGIN = 2,FUN = median,na.rm=TRUE),
    ...
    data_now <- c(apply(abs(group_ENV[[t]][,seq(1,12,2)]),MARGIN = 2,FUN = median,na.rm=TRUE),
                  apply(lm_ENV[[t]][,1:length(trait_names)],MARGIN = 2,FUN = median,na.rm=TRUE))
    names(data_now) <-gsub(names(data_now),pattern = "_obs",replacement = "") 
    names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
    data_now_obsENV <- as.data.frame(t(data_now))
    
    data_now <- c(apply(abs(group_TD[[t]][,seq(2,12,2)]),MARGIN = 2,FUN = median,na.rm=TRUE),
    ...
    data_now <- c(apply(abs(group_ENV[[t]][,seq(2,12,2)]),MARGIN = 2,FUN = median,na.rm=TRUE),
    ...
    data_now <- c(apply(abs(group_diss[[t]][,seq(1,12,2)]),MARGIN = 2,FUN = median,na.rm=TRUE),
                  apply(lm_diss[[t]][,1:4],MARGIN = 2,FUN = median,na.rm=TRUE))
    names(data_now) <-gsub(names(data_now),pattern = "_obs",replacement = "") 
    names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
    data_now_obs <- as.data.frame(t(data_now))
    
    data_now <- c(apply(abs(group_diss[[t]][,seq(2,12,2)]),MARGIN = 2,FUN = median,na.rm=TRUE),
                  apply(lm_diss[[t]][,5:8],MARGIN = 2,FUN = median,na.rm=TRUE))
    names(data_now) <-gsub(names(data_now),pattern = "_pred",replacement = "") 
    names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
    data_now_pred <- as.data.frame(t(data_now))
    
    data_now <- c(apply(abs(group_DissENV[[t]][,seq(2,12,2)]),MARGIN = 2,FUN = median,na.rm=TRUE),
    ...
    data_now <- c(apply(abs(group_DissENV[[t]][,seq(1,12,2)]),MARGIN = 2,FUN = median,na.rm=TRUE),
    ...
    data_now <- c(apply(abs(group_sim[[t]][,seq(1,12,2)]),MARGIN = 2,FUN = median,na.rm=TRUE),
                  apply(lm_sim[[t]][,1:4],MARGIN = 2,FUN = median,na.rm=TRUE))
    names(data_now) <-gsub(names(data_now),pattern = "_obs",replacement = "") 
    names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
    data_now_obs <- as.data.frame(t(data_now))
    
    data_now <- c(apply(abs(group_sim[[t]][,seq(2,12,2)]),MARGIN = 2,FUN = median,na.rm=TRUE),
                  apply(lm_sim[[t]][,5:8],MARGIN = 2,FUN = median,na.rm=TRUE))
    names(data_now) <-gsub(names(data_now),pattern = "_pred",replacement = "") 
    names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
    data_now_pred <- as.data.frame(t(data_now))
    
    data_now <- c(apply(abs(group_simENV[[t]][,seq(2,12,2)]),MARGIN = 2,FUN = median,na.rm=TRUE),
                  apply(lm_sim[[t]][,5:8],MARGIN = 2,FUN = median,na.rm=TRUE))
    names(data_now) <-gsub(names(data_now),pattern = "_pred",replacement = "") 
    names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
    data_now_predENV <- as.data.frame(t(data_now))
    
    data_now <- c(apply(abs(group_simENV[[t]][,seq(1,12,2)]),MARGIN = 2,FUN = median,na.rm=TRUE),
                  apply(lm_sim[[t]][,1:4],MARGIN = 2,FUN = median,na.rm=TRUE))
    names(data_now) <-gsub(names(data_now),pattern = "_obs",replacement = "") 
    names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
    data_now_obsENV <- as.data.frame(t(data_now))
...
data_chart <- data_chart[1:2,]
# ---------------------------------------------------------------------
# explanatory figure
# ---------------------------------------------------------------------
pdf(file=file.path(origin,"_2021","figures","Figure_6","Figure_6_SpiderExplainTD2.pdf"),width = 18,height=6)
  ...
  legend(0.5,10,legend = c("TD2","TD2_td","TD2_env"),col=colz_solid[1:3],lwd=8,lty=1,bty="n",cex=3)
