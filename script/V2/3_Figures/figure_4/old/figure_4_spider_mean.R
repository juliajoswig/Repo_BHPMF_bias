...
t=2
  ...
  library(fmsb)
pdf(file=file.path(origin,"_2021","figures","Figure_6","Figure_spider_mean.pdf"))#,width = 11,height=8
{
...
for(t in 1:6){
  data_now <- c(apply(group_TD[[t]][,seq(1,12,2)],MARGIN = 2,FUN = median,na.rm=TRUE),
                apply(lm_TD[[t]][,1:5],MARGIN = 2,FUN = median,na.rm=TRUE))
  ...
  data_now <- c(apply(group_ENV[[t]][,seq(2,12,2)],MARGIN = 2,FUN = median,na.rm=TRUE),
                apply(lm_ENV[[t]][,6:10],MARGIN = 2,FUN = median,na.rm=TRUE))
  names(data_now) <-gsub(names(data_now),pattern = "_pred",replacement = "") 
  names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
  data_now_predENV <- as.data.frame(t(data_now))
  
  res[[t]] <- abs(rbind(data_now_obs,data_now_pred,data_now_predENV))[,c(1:6)]
}
j=1
res2=matrix(NA,ncol=ncol(res[[1]]),nrow=nrow(res[[1]]))
for(j in 1:nrow(res[[1]])){
for(i in 1:ncol(res[[1]])){
  res2[j,i] <- mean(res[[1]][j,i],res[[2]][j,i],res[[3]][j,i],
                   res[[4]][j,i],res[[5]][j,i],res[[6]][j,i])
  }
  }

data_chart <- rbind(rep(0,ncol(data_plot)) , rep(1,ncol(data_plot)) , data.frame(res2))
colnames(data_chart) <- colnames(res[[1]])
par(mar=c(0,0,4,0),xpd = TRUE)
radarchart(data_chart,pcol = colz_solid,cglty = 1,cglwd = 2,axistype = 2,
           plty = c(1,1,1,1),plwd = c(10,8,8,8),pty = 16,axislabcol = "gray",
           #  pfcol = c(colz_alpha[2],NA,NA),
           title = "Total",vlcex = 3,calcex = 1.5,cex.main = 4,cglcol = "gray")
...
for(t in 1:6){  
  ...
  data_now <- c(apply(group_DissENV[[t]][,seq(1,12,2)],MARGIN = 2,FUN = median,na.rm=TRUE),
                apply(lm_DissENV[[t]][,1:5],MARGIN = 2,FUN = median,na.rm=TRUE))
  names(data_now) <-gsub(names(data_now),pattern = "_obs",replacement = "") 
  names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
  data_now_obsENV <- as.data.frame(t(data_now))
...
#           title = paste0("Total"),vlcex = 3,calcex = 1.5,cex.main = 5,cglcol = "gray")
...
names(res) <- colnames(res[[1]])
data_chart <- rbind(rep(0,ncol(data_plot)) , rep(1,ncol(data_plot)) , res2)
colnames(data_chart) <- colnames(res[[1]])
par(mar=c(0,0,4,0),xpd = TRUE)
radarchart(data_chart,pcol = colz_solid,cglty = 1,cglwd = 2,axistype = 2,
           plty = c(1,1,1,1),plwd = c(10,8,8,8),pty = 16,axislabcol = "gray",
           #  pfcol = c(colz_alpha[2],NA,NA),
           title = "Outliers",vlcex = 3,calcex = 1.5,cex.main = 4,cglcol = "gray")
#---------------------------------------------------------------------------------
...
radarchart(data_chart,pcol = colz_solid,cglty = 1,cglwd = 2,axistype = 2,
           plty = c(1,1,1,1),plwd = c(10,8,8,8),pty = 16,axislabcol = "gray",
           #  pfcol = c(colz_alpha[2],NA,NA),
           title = "Aligned",vlcex = 3,calcex = 1.5,cex.main = 4,cglcol = "gray")

}
dev.off()  


pdf(file=file.path(origin,"_2021","figures","Figure_4","Figure_4_3.pdf"),width = 11,height=8)
par(mar=c(2,2,4,2),xpd = TRUE)
layout(mat = matrix(c(1,1,1,2,
                      1,1,1,3),nrow=2,byrow = TRUE))
 ...
 pdf(file=file.path(origin,"_2021","figures","Figure_4","Figure_4_3.pdf"),width = 11,height=8)
  ...
  for(t in 1:6){
    ...
    for(t in 1:6){
      ...
      #data_plot[data_plot>1] <- 1
      data_chart <- rbind(rep(0,ncol(data_plot)) , rep(1,ncol(data_plot)) , data_plot )
      radarchart(data_chart,pcol = colz_solid,axistype = 2,plty = c(1,1,1,1),plwd = 8,pty = 16,axislabcol = "gray",
                 pfcol = c(colz_alpha[2],NA,NA),
      ...
      radarchart(data_chart,pcol = colz_solid,axistype = 2,plty = c(1,1,1,1),plwd = 8,pty = 16,axislabcol = "gray",
                 pfcol = c(colz_alpha[2],NA,NA),
                 title = paste0("Freaks"),vlcex = 2,calcex = 1.5,cex.main = 2,cglcol = "gray")
      text(0,0,trait_names[t])
      ...
      radarchart(data_chart,pcol = colz_solid,axistype = 2,plty = c(1,1,1,1),plwd = 8,pty = 16,axislabcol = "gray",
                 pfcol = c(colz_alpha[2],NA,NA),
                 title = paste0("Aligned"),vlcex = 2,calcex = 1.5,cex.main = 2,cglcol = "gray")
      text(0,0,trait_names[t])
    ...
    # explanatory figure
    # ---------------------------------------------------------------------
    pdf(file=file.path(origin,"_2021","figures","Figure_6","Figure_6_SpiderExplain.pdf"),width = 18,height=6)
    par(mar=c(0,0,0,0),xpd = TRUE,mfrow=c(1,2))
    data_chart[data_chart!=0] <- 0
    radarchart(data_chart,pcol = colz_solid,cglty = 1, 
               cglwd = 2,axistype = 3,plty = c(1,1,1,1),plwd = c(5,5,5,5),pty = 16,
               axislabcol = "black",
               #  pfcol = c(colz_alpha[2],NA,NA),
    ...
    legend(0.5,10,legend = c("TD","TD_td","TD_env"),col=colz_solid[1:3],lwd=8,lty=1,bty="n",cex=3)
    text(2.6,5,labels = "x: individual trait value",cex=2)
    text(5,4,labels = "Outlier: 25 most distant individuals to species mean",cex=2)
    text(5,3,labels = "Aligned: 25 most similar individuals to species mean",cex=2)
    #  text(4.8,2,labels = "lm(trait_n): residual of predicted target trait value ",cex=2)
    #  text(2.4,1,labels = "from lm, input trait_n",cex=2)
    dev.off()
