  ...
  par(mfrow=c(4,4))
  {
    ...
    
    {
      ...
      TD_sparse <- as.data.frame(read.csv(file.path(origin,"_2021","data","_runs",paste0("Rep_",RepNum),t_choice,paste0("p_80"),
                                                    "Obs_obs_TD","data","traitInfo.csv"),header=TRUE))[,-c(1,2)]
      TD_tax <- as.data.frame(read.table(file = file.path(origin,"_2021","data","_runs",paste0("Rep_",RepNum),t_choice,paste0("p_0"),
...
#      plot(TD_sparse_tx[,1],Env_sparse_tx[match(TD_tax$ID,Env_tax_c$ID),1])
    ...
    pdf(file=file.path(origin,"_2021","figures","Figure_2",paste0(txs[tx],"_boxplots_",t_choice,".pdf")),width=8,height=2.3,pointsize = 9)
    par(mfrow=c(1,6),mar=c(10,4,1,1))
    t=1
    ...
    for(t in 1:ncol(TD)){
      ...
      colnames(dat_plot)=  c("TD_td - TD",        "TD_td - TDsparse",      "TD_td - ExTD_sparse", "TD_ext - ExTD_sparse",  "TD_ext - TD")
