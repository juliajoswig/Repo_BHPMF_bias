...
TDtax <- read.csv(file = file.path(origin,"_2021","data","_runs",paste0("Rep_",RepNum),t_choice,paste0("p_",Percent),ObsOrTD,"data","taxInfo.csv"))
head(TDtax)
TD80 <- TD80[,-c(1,2)]
...
Envtax <- read.csv(file = file.path(origin,"_2021","data","_runs",paste0("Rep_",RepNum),t_choice,paste0("p_",Percent),ObsOrTD,"data","taxInfo.csv"))
head(Env)
head(Envtax)
dim(Env)
ID_env <- Env[,2]
...
# table
speciesTD <- unique(TDtax[,2])
m_Env <- matrix(NA,ncol=2,nrow=length(speciesTD))
m_TD <- matrix(NA,ncol=2,nrow=length(speciesTD))
m_TDtd <- matrix(NA,ncol=2,nrow=length(speciesTD))
m_TDenv <- matrix(NA,ncol=2,nrow=length(speciesTD))
m_Env[,1] <- speciesTD
m_TD[,1] <- speciesTD
m_TDtd[,1] <- speciesTD
m_TDenv[,1] <- speciesTD
