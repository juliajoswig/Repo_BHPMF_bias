...
res_ENV <- read.csv(file=file.path(origin,"_2021","data","analyses","Point_wise",RepNum,t_choice,ObsOrTD,Percent,paste0("res_2021.csv")))
  ...
  w=1
    ...
    colnames(bxpl_sp_op) <- rep(c("obs","pred","envelope"),6)
    colnames(bxpl_gen_op) <- rep(c("obs","pred","envelope"),6)
    colnames(bxpl_fam_op) <- rep(c("obs","pred","envelope"),6)
    colnames(bxpl_clad_op) <- rep(c("obs","pred","envelope"),6)
    colnames(bxpl_GF_op) <- rep(c("obs","pred","envelope"),6)
    colnames(bxpl_PFT_op) <- rep(c("obs","pred","envelope"),6)
    
    colz=c("#ef8a62","#ef8a62","#fddbc7","#fddbc7","#f7f7f7","#f7f7f7",
           "#d1e5f0","#d1e5f0","#67a9cf","#67a9cf","#2166ac","#2166ac")# no red "#b2182b","#b2182b",
  
    pdf(file=file.path(origin,"_2021","figures","Figure_2",paste0("Figure_2_Dist_Env.pdf")),width=20,height=8)
    {
      par(mar=c(4,0,4,0))
      ...
      for(t in 1:6){
        ...
        head(bxpl)
        boxplot((bxpl[,-seq(3,ncol(bxpl),3)]),ylim=c(0,ytop),ylab="CV pred - obs",col=colz,cex.main=5,
       ...
       # axis(side = 1,line = 3, at = seq(from=1.5,to = 12,by = 2),las=2,labels=trait_names,tick = FALSE,cex.axis=2)
        ...
        points(c(seq(2,12,2)),apply(bxpl[,seq(3,ncol(bxpl),3)],2,FUN=median,na.rm = TRUE),pch=16,col="white",cex=3)
        points(c(seq(2,12,2)),apply(bxpl[,seq(3,ncol(bxpl),3)],2,FUN=median,na.rm = TRUE),pch=16,col="#b2182b",cex=2)
        points(c(seq(2,12,2)),apply(bxpl[,seq(3,ncol(bxpl),3)],2,FUN=median,na.rm = TRUE),pch=8,col="#b2182b",cex=3)
      }
