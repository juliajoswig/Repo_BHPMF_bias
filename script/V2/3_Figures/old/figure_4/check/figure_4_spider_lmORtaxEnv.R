...
colz_solid=c(rgb(103/255,169/255,207/255),rgb(239/255,138/255,98/255),
             rgb(5/255,113/255,176/255),rgb(202/255,0/255,32/255))
...
for(t in 1:length(trait_names)){
  ix_trait=res$trait==trait_names[t]&res$nb_spec==1&res$nb_gen==1
...
t=1
for(t in 1:length(trait_names)){
  ix_trait=res$trait==trait_names[t]&res$nb_spec>10
  ix_trait[is.na(ix_trait)] <- FALSE
...
lm_muchTax<- lm_op 
group_muchTax <- group_op
vals_muchTax <- vals_op
...
t=1
for(t in 1:length(trait_names)){
  ix_trait=res$trait==trait_names[t]&res$dist_spec_obs_zlog>quantile(res$dist_spec_obs_zlog,probs = .9,na.rm = TRUE)
  ix_trait[is.na(ix_trait)] <- FALSE
...
t=1
for(t in 1:length(trait_names)){
  ix_trait=res$trait==trait_names[t]&res$dist_spec_obs_zlog<quantile(res$dist_spec_obs_zlog,probs = .2,na.rm = TRUE)
  ix_trait[is.na(ix_trait)] <- FALSE
...
t=1
for(t in 1:length(trait_names)){
  ix_trait=resENV$trait==trait_names[t]&resENV$nb_spec==1&res$nb_gen==1
  ix_trait[is.na(ix_trait)] <- FALSE
...
lm_noTaxENV<- lm_op 
group_noTaxENV <- group_op
vals_noTaxENV <- vals_op
...
lm_muchTaxENV<- lm_op 
group_muchTaxENV <- group_op
vals_muchTaxENV <- vals_op
...
t=1
  ...
  library(fmsb)
  pdf(file=file.path(origin,"_2021","figures","Figure_4","Figure_4_Spider_zloglmOrtaxEnv.pdf"),width = 19,height=5)
  par(mar=c(0,0,4,0),mfrow=c(1,4))
  for(t in 1:6){
    data_now <- c(apply(group_muchTax[[t]][,seq(1,12,2)],MARGIN = 2,FUN = median,na.rm=TRUE),
                  apply(lm_muchTax[[t]][,1:5],MARGIN = 2,FUN = median,na.rm=TRUE))
    ...
    data_now <- c(apply(group_muchTaxENV[[t]][,seq(1,12,2)],MARGIN = 2,FUN = median,na.rm=TRUE),
                  apply(lm_muchTaxENV[[t]][,1:5],MARGIN = 2,FUN = median,na.rm=TRUE))
    names(data_now) <-gsub(names(data_now),pattern = "_obs",replacement = "") 
    names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
    data_now_obsENV <- as.data.frame(t(data_now))
    
    data_now <- c(apply(group_muchTax[[t]][,seq(2,12,2)],MARGIN = 2,FUN = median,na.rm=TRUE),
                  apply(lm_muchTax[[t]][,6:10],MARGIN = 2,FUN = median,na.rm=TRUE))
    names(data_now) <-gsub(names(data_now),pattern = "_pred",replacement = "") 
    names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
    data_now_pred <- as.data.frame(t(data_now))
    
    data_now <- c(apply(group_muchTaxENV[[t]][,seq(2,12,2)],MARGIN = 2,FUN = median,na.rm=TRUE),
                  apply(lm_muchTaxENV[[t]][,6:10],MARGIN = 2,FUN = median,na.rm=TRUE))
    names(data_now) <-gsub(names(data_now),pattern = "_pred",replacement = "") 
    names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
    ...
    radarchart(data_chart,pcol = colz_solid[c(1,2,4)],axistype = 2,plty = c(1,1,2),plwd = 8,pty = 32,axislabcol = "gray",
               title = "Many spec and gen",vlcex = 1.8,calcex = 1.5,cex.main = 3)
    text(0,0,trait_names[t])
    
    #--------------------------------------------------------------------
    data_now <- c(apply(group_noTax[[t]][,seq(1,12,2)],MARGIN = 2,FUN = median,na.rm=TRUE),
                  apply(lm_noTax[[t]][,1:5],MARGIN = 2,FUN = median,na.rm=TRUE))
    names(data_now) <-gsub(names(data_now),pattern = "_obs",replacement = "") 
    names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
    data_now_obs <- as.data.frame(t(data_now))
    
    data_now <- c(apply(group_noTax[[t]][,seq(2,12,2)],MARGIN = 2,FUN = median,na.rm=TRUE),
                  apply(lm_noTax[[t]][,6:10],MARGIN = 2,FUN = median,na.rm=TRUE))
    names(data_now) <-gsub(names(data_now),pattern = "_pred",replacement = "") 
    names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
    data_now_pred <- as.data.frame(t(data_now))
    
    
    data_now <- c(apply(group_noTaxENV[[t]][,seq(2,12,2)],MARGIN = 2,FUN = median,na.rm=TRUE),
                  apply(lm_noTaxENV[[t]][,6:10],MARGIN = 2,FUN = median,na.rm=TRUE))
    names(data_now) <-gsub(names(data_now),pattern = "_pred",replacement = "") 
    names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
    data_now_predENV <- as.data.frame(t(data_now))
    ...
    data_chart <- rbind(rep(0,ncol(data_plot)) , rep(1,ncol(data_plot)) , data_plot)
    radarchart(data_chart,pcol = colz_solid[c(1,2,4)],axistype = 2,plty = c(1,1,2),plwd = 8,pty = 32,axislabcol = "gray",
               title = "Few spec and gen",vlcex = 1.8,calcex = 1.5,cex.main = 3)
    text(0,0,trait_names[t])
    ...
    data_now <- c(apply(group_diss[[t]][,seq(1,12,2)],MARGIN = 2,FUN = median,na.rm=TRUE),
                  apply(lm_diss[[t]][,1:5],MARGIN = 2,FUN = median,na.rm=TRUE))
    names(data_now) <-gsub(names(data_now),pattern = "_obs",replacement = "") 
    names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
    data_now_obs <- as.data.frame(t(data_now))
    
    data_now <- c(apply(group_diss[[t]][,seq(2,12,2)],MARGIN = 2,FUN = median,na.rm=TRUE),
                  apply(lm_diss[[t]][,6:10],MARGIN = 2,FUN = median,na.rm=TRUE))
    names(data_now) <-gsub(names(data_now),pattern = "_pred",replacement = "") 
    names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
    data_now_pred <- as.data.frame(t(data_now))
    
    data_now <- c(apply(group_DissENV[[t]][,seq(2,12,2)],MARGIN = 2,FUN = median,na.rm=TRUE),
                  apply(lm_DissENV[[t]][,6:10],MARGIN = 2,FUN = median,na.rm=TRUE))
    names(data_now) <-gsub(names(data_now),pattern = "_pred",replacement = "") 
    names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
    data_now_predENV <- as.data.frame(t(data_now))
    ...
    data_chart <- rbind(rep(0,ncol(data_plot)) , rep(1,ncol(data_plot)) , data_plot)
    radarchart(data_chart,pcol = colz_solid[c(1,2,4)],axistype = 2,plty = c(1,1,2),plwd = 8,pty = 32,axislabcol = "gray",
               title = "High spec distance",vlcex = 1.8,calcex = 1.5,cex.main = 3,)
    text(0,0,trait_names[t])
    ...
    data_now <- c(apply(group_sim[[t]][,seq(1,12,2)],MARGIN = 2,FUN = median,na.rm=TRUE),
                  apply(lm_sim[[t]][,1:5],MARGIN = 2,FUN = median,na.rm=TRUE))
    names(data_now) <-gsub(names(data_now),pattern = "_obs",replacement = "") 
    names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
    data_now_obs <- as.data.frame(t(data_now))
    
    data_now <- c(apply(group_sim[[t]][,seq(2,12,2)],MARGIN = 2,FUN = median,na.rm=TRUE),
                  apply(lm_sim[[t]][,6:10],MARGIN = 2,FUN = median,na.rm=TRUE))
    names(data_now) <-gsub(names(data_now),pattern = "_pred",replacement = "") 
    names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
    data_now_pred <- as.data.frame(t(data_now))
    ...
    data_chart <- rbind(rep(0,ncol(data_plot)) , rep(1,ncol(data_plot)) , data_plot)
    radarchart(data_chart,pcol = colz_solid[c(1,2,4)],axistype = 2,plty = c(1,1,2),plwd = 8,pty = 32,axislabcol = "gray",
               title = "Low spec distance",vlcex = 1.8,calcex = 1.5,cex.main = 3,)
    text(0,0,trait_names[t])
