...
#  colz2 =out$colz2
  ...
  pdf(file=file.path(origin,"plots","figure_5",paste0("Envelope_Sil",ObsSpec_now,".pdf")),width=10,height=3.5)
...
## 
  ...
  pdf(file=file.path(origin,"plots","figure_5","Envelope_correlation.pdf"),width=8,height=4)
  par(mfrow=c(1,2))
  i=1
  for(i in 1){
    ObsSpec_nowTD <- TD_choices[i]
  ...
  for(i in 1:2){
    ObsSpec_nowTD <- TD_choices[i]
    ObsSpec_now <- TD_choices[i+2]
pdf(file=file.path(origin,"plots","figure_5",paste0("Envelope_rmse",ObsSpec_now,".pdf")),width=8,height=4)
par(mfrow=c(1,2))
    j=2
    for(j in 1:2){
      test_data_now <- tsubs[j]
  ...
  for(i in 1:2){
    ObsSpec_nowTD <- TD_choices[i]
    ObsSpec_now <- TD_choices[i+2]
    pdf(file=file.path(origin,"plots","figure_5",paste0("Envelope_rmse_both",ObsSpec_now,".pdf")),
        width=10,height=3)
    par(mfrow=c(1,4))
    j=2
    for(j in 1:2){
