...
for(GapPercent in gappercents){
  ...
  pdf(file=file.path(origin,"figures","figure_4","figure_4_a.pdf"),width=12,height=4)
  par(mfrow=c(2,5))
  ix=5:(5+5)
  print(colnames(R)[ix])
  group_nms=c("Species","Genus","Family","PG","GF","PFT")
  t=1
  tc=1
  g=2
  for(g in 1:length(group_nms)){
    for(t in 1:length(ix)){
    for(tc in 1:4){
      
      group_nm=group_nms[g]
      
      if(TD_choices[tc]%in%c("Spec_spec_TD","Spec_spec")&group_nm!="Species"|
         !TD_choices[tc]%in%c("Spec_spec_TD","Spec_spec")){
    ix_org=R$Group==group_nm&R$Gaps=="org"&R$TD_choice==TD_choices[tc]
    ix_0=R$Group==group_nm&R$Gaps=="0"&R$TD_choice==TD_choices[tc]
    ix_1=R$Group==group_nm&R$Gaps=="1"&R$TD_choice==TD_choices[tc]
    ix_5=R$Group==group_nm&R$Gaps=="5"&R$TD_choice==TD_choices[tc]
    ix_10=R$Group==group_nm&R$Gaps=="10"&R$TD_choice==TD_choices[tc]
    ix_20=R$Group==group_nm&R$Gaps=="20"&R$TD_choice==TD_choices[tc]
    ix_30=R$Group==group_nm&R$Gaps=="30"&R$TD_choice==TD_choices[tc]
    ix_40=R$Group==group_nm&R$Gaps=="40"&R$TD_choice==TD_choices[tc]
    ix_50=R$Group==group_nm&R$Gaps=="50"&R$TD_choice==TD_choices[tc]
    ix_60=R$Group==group_nm&R$Gaps=="60"&R$TD_choice==TD_choices[tc]
    ix_70=R$Group==group_nm&R$Gaps=="70"&R$TD_choice==TD_choices[tc]
    gc=6
    ...
    plot(dat_obs[,1],dat_pred[,1],xlim=c(-5,5),ylim=c(-5,5),pch=16,col=colz1[8],xlab="Observed cova",ylab="Predicted cova",main=paste0(group_nm," ",colnames(R)[ix[t]]))     
    #plot(dat_obs[,2],dat_pred[,2],xlim=c(-5,5),ylim=c(-5,5),pch=16,col=colz1[8],xlab="Observed cova",ylab="Predicted cova",main="0%")
    plot(dat_obs[,3],dat_pred[,3],xlim=c(-5,5),ylim=c(-5,5),pch=16,col=colz1[8],xlab="Observed cova",ylab="Predicted cova",main=paste0(TD_choices[tc]," 1%"))
...
pdf(file=file.path(origin,"plots","figure_4","figure_4_c.pdf"),width=10,height=10)
par(mfrow=c(3,4))
t=1
{
  group_nm="Species"
