...
pattern_now="RMSE"
ix2=res[,colnames(res)=="TraitChoice"]=="data_2"&res[,colnames(res)=="Obs_or_Spec"]=="Obs_obs_TD"
...
dat_plot2 <- aggregate(dat_rmse2,by=list(Percent2),FUN=mean,na.rm = TRUE)
...
dat_plotENV[,colnames(dat_plotENV)=="RMSE_zlog_Total"]
dat_plot2[,colnames(dat_plot2)=="RMSE_gap_zlog_Total"]
dat_plotENV[,colnames(dat_plotENV)=="RMSE_gap_zlog_Total"]

  pdf(file=file.path(origin,"_2021","Figures","Figure_1","Fig_S1_RMSE_zlog.pdf"),width=10,height=8)
  ...
  pdf(file=file.path(origin,"_2021","Figures","Figure_1","Fig_S1_RMSE.pdf"),width=10,height=8)
      par(mfrow=c(1,1),mar=c(7,7,2,2))
      i=2
      plot(dat_plot2[,c(1,i)],col=colz[i],pch=16,cex=.1,cex.lab=3,cex.axis=3,
