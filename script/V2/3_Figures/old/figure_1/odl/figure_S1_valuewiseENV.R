...
pdf(file=file.path(origin,"_2021","figures","Figure_1","Fig_S1_ValueWiseENV.pdf"),width=10,height =8)
...
for(t in 1:length(trait_names)){
  ...
  boxplot(bxpl,ylim=c(limmin[t],limmax[t]),ylab=paste0(trait_names[t],"[",units[t],"]"),width=1,las=2,col=colz[t],cex.axis=2,cex.lab=3,tick = FALSE,frame=FALSE)
...
pdf(file=file.path(origin,"_2021","figures","figure_1","Fig_S1_ValueWiseGAPSonlyENV.pdf"),width=10,height =8)
par(mfrow=c(1,1),mar=c(4,4,2,2))
par(mfrow=c(1,7),mar=c(0,6,0,0))
