...
cnfd <- read.table(file.path(origin,"_2021","data","_runs",paste0("Rep_",RepNum),t_choice,paste0("p_",Percent),ObsOrTD,"data","std.csv"), sep="\t",header=TRUE)
  ...
  for(t in 1:length(trait_names)){
    ...
    dat_plot <- rbind(dat_plot,cbind(abs(res_now$value_pred_zlog-res_now$value_obs_zlog),cnfd[,t]))
  }
  dat_lm<- data.frame(dat_plot)
  lm_now <- lm(X2~X1,dat_lm)
  plot(dat_plot,xlim=c(0,3),ylim=c(0,1.2),pch=16,col=colz_alpha[1],xlab="RMSE",ylab="Confidence [sd]",cex.lab=2)
  abline(lm_now$coefficients,lwd=2)
  lm_smr <- summary(lm_now)
  text(1,.9,labels = paste0("Pearson: ",round(cor(dat_plot,use = "pairwise.complete.obs")[1,2],digits = 2)),cex = 2)
  text(1,.8,labels = paste0("R2 (adj): ", round(lm_smr$adj.r.squared,digits = 2)),cex = 2)
  ...
  for(t in 1:length(trait_names)){
  res_now=res[res$trait==trait_names[t],]
  dat_plot <- rbind(dat_plot,cbind(abs(res_now$dist_spec_pred_zlog-res_now$dist_spec_obs_zlog),cnfd[,t]))
  }
  dat_lm<- data.frame(dat_plot)
  lm_now <- lm(X2~X1,dat_lm)
  plot(dat_plot,xlim=c(0,2.5),ylim=c(0,1.2),pch=16,col=colz_alpha[1],xlab="Deviation pred-obs",ylab="Confidence [sd]",cex.lab=2)
  abline(lm_now$coefficients,lwd=2)
  lm_smr <- summary(lm_now)
  text(1,.9,labels = paste0("Pearson: ",round(cor(dat_plot,use = "pairwise.complete.obs")[1,2],digits = 2)),cex = 2)
  text(1,.8,labels = paste0("R2 (adj): ", round(lm_smr$adj.r.squared,digits = 2)),cex = 2)
  ...
  for(t in 1:length(trait_names)){
    res_now=res[res$trait==trait_names[t],]
    dat_plot <- rbind(dat_plot,cbind(abs(res_now$dist_gen_pred_zlog-res_now$dist_gen_obs_zlog),cnfd[,t]))
  }
  dat_lm<- data.frame(dat_plot)
  lm_now <- lm(X2~X1,dat_lm)
  plot(dat_plot,xlim=c(0,2.5),ylim=c(0,1.2),pch=16,col=colz_alpha[1],xlab="Deviation pred-obs",ylab="Confidence [sd]",cex.lab=2)
  ...
  for(t in 1:length(trait_names)){
    res_now=res[res$trait==trait_names[t],]
    dat_plot <- rbind(dat_plot,cbind(abs(res_now$dist_fam_pred_zlog-res_now$dist_fam_obs_zlog),cnfd[,t]))
  }
  dat_lm<- data.frame(dat_plot)
  lm_now <- lm(X2~X1,dat_lm)
  plot(dat_plot,xlim=c(0,2.5),ylim=c(0,1.2),pch=16,col=colz_alpha[1],xlab="Deviation pred-obs",ylab="Confidence [sd]",cex.lab=2)
  ...
  for(t in 1:length(trait_names)){
    res_now=res[res$trait==trait_names[t],]
    dat_plot <- rbind(dat_plot,cbind(abs(res_now$dist_clad_pred_zlog-res_now$dist_clad_obs_zlog),cnfd[,t]))
  }
  dat_lm<- data.frame(dat_plot)
  lm_now <- lm(X2~X1,dat_lm)
  plot(dat_plot,xlim=c(0,2.5),ylim=c(0,1.2),pch=16,col=colz_alpha[1],xlab="Deviation pred-obs",ylab="Confidence [sd]",cex.lab=2)
