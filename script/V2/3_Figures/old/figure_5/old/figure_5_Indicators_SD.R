...
xs <- round(res$dist_gen_obs_zlog/max(res$dist_gen_obs_zlog,na.rm = TRUE),digits = 3)
...
pdf(file=file.path(origin,"_2021","figures","Figure_5","Figure_5_dist_gen.pdf"),width=20,height=5)
...
colz_now=colz_rgbinv
{
  ...
  for(t in 1:length(trait_names)){
    ...
    dat_plot <- rbind(dat_plot,cbind(abs(res_now$value_pred_zlog-res_now$value_obs_zlog),res_now$dist_gen_obs_zlog))
  }
  dat_lm<- data.frame(dat_plot)
  lm_now <- lm(X2~X1,dat_lm)
  plot(dat_plot,pch=16,col=colz_now,xlab="Pred-obs",ylab="Distance to co-genus plants",cex.lab=2)
  ...
  for(t in 1:length(trait_names)){
    res_now=res[res$trait==trait_names[t],]
    dat_plot <- rbind(dat_plot,cbind(abs(res_now$dist_spec_pred_zlog-res_now$dist_spec_obs_zlog),res_now$dist_gen_obs_zlog))
  }
  dat_lm<- data.frame(dat_plot)
  lm_now <- lm(X2~X1,dat_lm)
  plot(dat_plot,pch=16,col=colz_now,xlab="Deviation pred-obs (sp)",ylab="Distance to co-genus plants",cex.lab=2)
  abline(lm_now$coefficients,lwd=2)
  lm_smr <- summary(lm_now)
  ...
  for(t in 1:length(trait_names)){
    res_now=res[res$trait==trait_names[t],]
    dat_plot <- rbind(dat_plot,cbind(abs(res_now$dist_gen_pred_zlog-res_now$dist_gen_obs_zlog),res_now$dist_gen_obs_zlog))
  }
  dat_lm<- data.frame(dat_plot)
  lm_now <- lm(X2~X1,dat_lm)
  plot(dat_plot,pch=16,col=colz_now,xlab="Deviation pred-obs (gen)",ylab="Distance to co-genus plants",cex.lab=2)
  abline(lm_now$coefficients,lwd=2)
  lm_smr <- summary(lm_now)
  ...
  for(t in 1:length(trait_names)){
    res_now=res[res$trait==trait_names[t],]
    dat_plot <- rbind(dat_plot,cbind(abs(res_now$dist_fam_pred_zlog-res_now$dist_fam_obs_zlog),res_now$dist_gen_obs_zlog))
  }
  dat_lm<- data.frame(dat_plot)
  lm_now <- lm(X2~X1,dat_lm)
  plot(dat_plot,pch=16,col=colz_now,xlab="Deviation pred-obs (fam)",ylab="Distance to co-genus plants",cex.lab=2)
  abline(lm_now$coefficients,lwd=2)
  lm_smr <- summary(lm_now)
  ...
  for(t in 1:length(trait_names)){
    res_now=res[res$trait==trait_names[t],]
    dat_plot <- rbind(dat_plot,cbind(abs(res_now$dist_clad_pred_zlog-res_now$dist_clad_obs_zlog),res_now$dist_gen_obs_zlog))
  }
  dat_lm<- data.frame(dat_plot)
  lm_now <- lm(X2~X1,dat_lm)
  plot(dat_plot,pch=16,col=colz_now,xlab="Deviation pred-obs (clad)",ylab="Distance to co-genus plants",cex.lab=2)
  abline(lm_now$coefficients,lwd=2)
  lm_smr <- summary(lm_now)
  text(.8,2.0,labels = paste0("Pearson: ",round(cor(dat_plot,use = "pairwise.complete.obs")[1,2],digits = 2)),cex = 2)
  text(.8,1.5,labels = paste0("R2 (adj): ", round(lm_smr$adj.r.squared,digits = 2)),cex = 2)
