...
names(covaG)[2] <- "cluster"
tot_obs <- read.csv(file.path(origin, "data_output","CoVa","tot_obs.csv"))
tot_pred <- read.csv(file.path(origin, "data_output","CoVa","tot_pred.csv"))

spec_R <- covaR[covaR$Group=="Species",]
spec_m <- merge(tot_spec,spec_R,by = "cluster")
...
#for(td in 1:4){
  ...
  R_now=tot_obs
  ag_now=as.matrix(R_now[,-which(colnames(R_now)%in%c("X","group","cluster"))]);  mode(ag_now) <- "numeric"
  Tobs <- aggregate(x=ag_now,
                    by=list(Group=R_now$group,cluster=R_now$cluster),FUN=new_mean_fun)
  
  R_now=tot_pred
  ag_now=as.matrix(R_now[,-which(colnames(R_now)%in%c("X","group","cluster"))]);  mode(ag_now) <- "numeric"
  Tpred <- aggregate(x=ag_now,
                     by=list(Group=R_now$group,cluster=R_now$cluster),FUN=new_mean_fun)
  
  Ge <- merge(G,Tobs,by="cluster")
  Re <- merge(R,Tobs,by="cluster")

  
#  pdf(file=file.path(origin,"figures","figure_5",paste0("figure_5_f_",TD_choice,".pdf")),
#      width=12,height=3.5)
  par(mfrow=c(1,1),mar=c(6,2,5,2))
  t=1
  ix=5:9
  #for(t in 1:length(ix)){
    group_nm="Species"
    Ge$Group
    ix_org=Ge$Group.x==group_nm&Ge$Gaps=="org"
    ix_0=Ge$Group.x==group_nm&Ge$Gaps=="0"
    ix_1=Ge$Group.x==group_nm&Ge$Gaps=="1"
    ix_5=Ge$Group.x==group_nm&Ge$Gaps=="5"
    ix_10=Ge$Group.x==group_nm&Ge$Gaps=="10"
    ix_20=Ge$Group.x==group_nm&Ge$Gaps=="20"
    ix_30=Ge$Group.x==group_nm&Ge$Gaps=="30"
    ix_40=Ge$Group.x==group_nm&Ge$Gaps=="40"
    ix_50=Ge$Group.x==group_nm&Ge$Gaps=="50"
    ix_60=Ge$Group.x==group_nm&Ge$Gaps=="60"
    ix_70=Ge$Group.x==group_nm&Ge$Gaps=="70"
    #|ix_1|ix_5|ix_10|ix_20|ix_30|ix_40|ix_50|ix_60|ix_70
    G70 <- Ge[ix_0,]
    Gorg <- Ge[ix_org,]
    #plot(G70$SLA.x,G70$SLA.y,xlim=c(-10,10),ylim=c(-10,10))
    plot(G70$SLA.y,G70$SLA.x-Gorg$SLA.x,xlim=c(-10,10),ylim=c(-10,10))
    
    G70 <- Ge[ix_1,]
    #points(G70$SLA.x,G70$SLA.y,xlim=c(-10,10),ylim=c(-10,10),col="gray")
    points(G70$SLA.y,G70$SLA.x-Gorg$SLA.x,xlim=c(-10,10),ylim=c(-10,10),col="gray")
    
    G70 <- Ge[ix_5,]
    #points(G70$SLA.x,G70$SLA.y,xlim=c(-10,10),ylim=c(-10,10),col="red")
    points(G70$SLA.y,G70$SLA.x-Gorg$SLA.x,xlim=c(-10,10),ylim=c(-10,10),col="red")
    
    G70 <- Ge[ix_10,]
    #points(G70$SLA.x,G70$SLA.y,xlim=c(-10,10),ylim=c(-10,10),col="blue")
    points(G70$SLA.y,G70$SLA.x-Gorg$SLA.x,xlim=c(-10,10),ylim=c(-10,10),col="blue")
    
    G70 <- Ge[ix_20,]
    #points(G70$SLA.x,G70$SLA.y,xlim=c(-10,10),ylim=c(-10,10),col="lightblue")
    points(G70$SLA.y,G70$SLA.x-Gorg$SLA.x,xlim=c(-10,10),ylim=c(-10,10),col="lightblue")
    
    G70 <- Ge[ix_30,]
    #points(G70$SLA.x,G70$SLA.y,xlim=c(-10,10),ylim=c(-10,10),col="lightblue")
    points(G70$SLA.y,G70$SLA.x-Gorg$SLA.x,xlim=c(-10,10),ylim=c(-10,10),col="darkblue")
    
    G70 <- Ge[ix_40,]
    #points(G70$SLA.x,G70$SLA.y,xlim=c(-10,10),ylim=c(-10,10),col="lightblue")
    points(G70$SLA.y,G70$SLA.x-Gorg$SLA.x,xlim=c(-10,10),ylim=c(-10,10),col="orange")
    
    #------------------------------------------------------------------------
    
    pairs(cbind(G70$SLA.x,G70$SLA.y,G70$SLA.x-Gorg$SLA.x,Gorg$SLA.x))
    
    colnames(Ge)
    t=5
    p=16
    dat_datsetdist = cbind(Ge[ix_1,t]-Ge[ix_org,t],
                       Ge[ix_5,t]-Ge[ix_org,t],
                       Ge[ix_10,t]-Ge[ix_org,t],
                       Ge[ix_20,t]-Ge[ix_org,t],
                       Ge[ix_40,t]-Ge[ix_org,t],
                       Ge[ix_50,t]-Ge[ix_org,t],
                       Ge[ix_70,t]-Ge[ix_org,t])
    dat_envelope = cbind(Ge[ix_org,t+p],
                       Ge[ix_1,t+p],Ge[ix_5,t+p],Ge[ix_10,t+p],
                       Ge[ix_20,t+p],Ge[ix_40,t+p],
                       Ge[ix_50,t+p],Ge[ix_70,t+p])
    ...
    cbind(dat_envelope[,1],dat_datsetdist[,1])
