...
pdf(file=file.path(origin,"figures","figure_5","figure_5_e.pdf"),width = 16,height=4)
par(mfrow=c(1,6),mar=c(5,5,5,2))
col_env=c("#edf8e9","#bae4b3","#74c476","#238b45")
cexl=1.7  
cexm=1.7
tr=1
for(tr in 1:ncol(spec_obs)){
  boxplot(cbind(spec_obs[,tr],spec_pred[,tr]-spec_obs[,tr]),cex.lab=cexl,cex.main=cexm,
          main=paste0("Species envelope "),ylab="Deviation cova pred-obs",
          ylim=c(-3,3),xaxt="n",col=col_env)
  axis(3,at = 1,labels = colnames(spec_obs)[tr],tick = FALSE,line = -1,cex=cexm,cex.axis=cexm)
  axis(1,at = 1:2,labels = c("observed","predicted"),las=2)
  abline(h=0,col="red")
  
  boxplot(cbind(gen_obs[,tr],gen_pred[,tr]-gen_obs[,tr]),cex.lab=cexl,cex.main=cexm,
          main=paste0("Genera envelope "),ylab="Deviation cova pred-obs",
          ylim=c(-3,3),xaxt="n",col=col_env)
  axis(3,at = 1,labels = colnames(spec_obs)[tr],tick = FALSE,line = -1,cex=cexm,cex.axis=cexm)
  axis(1,at = 1:2,labels = c("observed","predicted"),las=2)
  abline(h=0,col="red")
  
  boxplot(cbind(fam_obs[,tr],fam_pred[,tr]-fam_obs[,tr]),cex.lab=cexl,cex.main=cexm,
          main=paste0("Families envelope "),ylab="Deviation cova pred-obs",
          ylim=c(-10,10),xaxt="n",col=col_env)
  axis(3,at = 1,labels = colnames(spec_obs)[tr],tick = FALSE,line = -1,cex=cexm,cex.axis=cexm)
  axis(1,at = 1:2,labels = c("observed","predicted"),las=2)
  abline(h=0,col="red")
  
  boxplot(cbind(pg_obs[,tr],pg_pred[,tr]-pg_obs[,tr]),cex.lab=cexl,cex.main=cexm,
          main=paste0("Phylo groups envelope "),ylab="Deviation cova pred-obs",
          ylim=c(-10,10),xaxt="n",col=col_env)
  axis(3,at = 1,labels = colnames(spec_obs)[tr],tick = FALSE,line = -1,cex=cexm,cex.axis=cexm)
  axis(1,at = 1:2,labels = c("observed","predicted"),las=2)
  abline(h=0,col="red")
  ...
  boxplot(cbind(gf_obs[,tr],gf_pred[,tr]-gf_obs[,tr]),cex.lab=cexl,cex.main=cexm,
          main=paste0("Growth forms envelope "),ylab="Deviation cova pred-obs",
          ylim=c(-10,10),xaxt="n",col=col_env)
  axis(3,at = 1,labels = colnames(spec_obs)[tr],tick = FALSE,line = -1,cex=cexm,cex.axis=cexm)
  axis(1,at = 1:2,labels = c("observed","predicted"),las=2)
  abline(h=0,col="red")
  
  boxplot(cbind(pft_obs[,tr],pft_pred[,tr]-pft_obs[,tr]),cex.lab=cexl,cex.main=cexm,
          main=paste0("PFTs envelope "),ylab="Deviation cova pred-obs",
          ylim=c(-10,10),xaxt="n",col=col_env)
  axis(3,at = 1,labels = colnames(spec_obs)[tr],tick = FALSE,line = -1,cex=cexm,cex.axis=cexm)
  axis(1,at = 1:2,labels = c("observed","predicted"),las=2)
  abline(h=0,col="red")
...
i=1
j=2
pdf(file=file.path(origin,"figures","figure_5","figure_5_f.pdf"),width=5,height=4)
par(mar=c(5,5,1,1))
  plot(1:10,col="white",ylim=c(-1,1),xlim=c(-1,1),cex.lab=1.2,
       xlab="Pearson observed",ylab="Deviation pred-observed")
  for(i in 1:ncol(cor_obs)){
    for(j in 1:ncol(cor_obs)){
      if(i!=j){
    points(x=cor_obs[i,j],y=c(cor_pred[i,j]-cor_obs[i,j]),pch=16,col=col_env[4])
...
library(psych)
pdf(file=file.path(origin,"figures","figure_5","EnvelopeGuidePairs_obs.pdf"),width=10,height=10)
