...
#-------------------------------------------------------------------
  ...
  list.files(file.path(origin,"_2021","data","_runs",paste0("Rep_",RepNum),t_choice,"p_0",ObsOrTD,"data"))
  # TD
  ObsOrTD="Obs_obs_TD"
...
#  Env_PFT <- Env[which(Envfun[,7]%in%TDtax[,7]),]
 ...
 for(cl in 2:7){
  ...
  setwd(origin)
  list.files()
#  pdf(file=file.path(origin,"_2021","figures","Figure_5",paste0("Figure_S_Silouette.pdf")),width=20,height=10)
  pdf(file=file.path("_2021","figures","Figure_5",paste0("Figure_S_Silouette_r.pdf")),width=20,height=10)
  par(mar=c(17,0,4,0),mfrow=c(1,7))
  {
    ...
    for(t in 1:6){
      ...
      axis(1,at = 1:ncol(dat_now),labels = c("TD2","TD2_td","TD2_ext"),las=2,cex.axis=4.5,tick=FALSE)
