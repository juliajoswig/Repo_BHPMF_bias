...
for(Percent in gappercents){
  ...
  #-------------------------------------------------------------------
  {
    ...
    if(resdo|!file.exists(file.path(originData,"analyses","Point_wise",
                                    RepNum,t_choice,ObsOrTD,Percent,paste0("res_2022_12.csv")))){
    ...
    res <- read.csv(file=file.path(originData,"analyses","Point_wise",RepNum,t_choice,ObsOrTD,Percent,paste0("res_2022_12.csv")))
  }
    ...
    for(t1 in 1:ncol(traitInfo_obs)){
        for(t in t){
          if(t1!=t){
            ...
            # build model from respective data
            #dat_now=as.data.frame(traitInfo_pred[,c(t1,t)])
            names(dat_now) <- c("t1","t")
            # lm produced from NONsparse data!
            # now, we prepare the trait-trait lm from respective data set. NOT observed.
          ...
          if(t1!=t){
            ...
            # build model from observed data
            dat_now=as.data.frame(traitInfo_obs[,c(t1,t)])
            # build model from respective data (here observed)
            #dat_now=as.data.frame(traitInfo_obs[,c(t1,t)])
            names(dat_now) <- c("t1","t")
  ...
  write.csv(res,file=file.path(originData,"analyses","Point_wise",RepNum,t_choice,ObsOrTD,Percent,paste0("res_2022_12.csv")))
