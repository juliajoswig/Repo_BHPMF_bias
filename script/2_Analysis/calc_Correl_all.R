  ...
  for(RepNum in 1:3){
    for(td in 1:2){
      ...
      for(p in 1:length(gappercents)){
        ...
        path_obs_zlog <- file.path(origin,"_2021","data","_runs",paste0("Rep_",RepNum),t_choice,paste0("p_",Percent),ObsOrTD,"data","traitInfoTD_obs_REzlog.csv")
        ...
        # create folder
              ...
              if(!file.exists(file.path(origin,"_2021","data","analyses","Correl"))){
                dir.create(file.path(origin,"_2021","data","analyses","Correl"))}
              if(!file.exists(file.path(origin,"_2021","data","analyses","Correl",t_choice))){
                dir.create(file.path(origin,"_2021","data","analyses","Correl",t_choice))}
              if(!file.exists(file.path(origin,"_2021","data","analyses","Correl",t_choice,ObsOrTD))){
                dir.create(file.path(origin,"_2021","data","analyses","Correl",t_choice,ObsOrTD))}
              if(!file.exists(file.path(origin,"_2021","data","analyses","Correl",t_choice,ObsOrTD,Percent))){
                dir.create(file.path(origin,"_2021","data","analyses","Correl",t_choice,ObsOrTD,Percent))}
              if(!file.exists(file.path(origin,"_2021","data","analyses","Correl",t_choice,ObsOrTD,Percent,RepNum))){
                dir.create(file.path(origin,"_2021","data","analyses","Correl",t_choice,ObsOrTD,Percent,RepNum))}
        print(Percent)
        print(cor_all)
        
        write.table(cor_all, file = file.path(origin,"_2021","data","analyses","Correl",t_choice,ObsOrTD,Percent,RepNum,paste0("Correl.csv")),
                    sep = ",",row.names = TRUE,col.names = TRUE)
        write.table(cor_zlog, file = file.path(origin,"_2021","data","analyses","Correl",t_choice,ObsOrTD,Percent,RepNum,paste0("Correl_zlog.csv")),
                    sep = ",",row.names = TRUE,col.names = TRUE)
