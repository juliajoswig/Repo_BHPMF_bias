...
data_rainfor <- data_rainfor[rowSums(!is.na(data_rainfor[23:ncol(data_rainfor)]))!=0,]
...
data_guido <- data_guido[rowSums(!is.na(data_guido[23:ncol(data_guido)]))!=0,]
...
for(RepNum in 1:3){
  ...
  for(td in 1:2){
    ...
    for(p in 1:length(gappercents)){
      ...
      # retain 1 per row -> do maximum sparsity
      # TRUE = value i.e. FALSE = NA
      ...
      #base % gaps:
      round(sum(ix_NA2)/(sum(ix_NA2)+sum(!ix_NA2)),digits = 2)
      
      Percent = gappercents[p]
      total_vals=(ncol(data_now)-1)*nrow(data_now)
      nb_total_entries = round(total_vals*((100-Percent)/100),digits = 0) # nb of values to keep
      nb_entries_add <- nb_total_entries-sum(ix_NA) # nb of values to add to base entries
      if(nb_entries_add<=0){nb_entries_add=1}
      length(as.vector(ix_NA2[!ix_NA])) # 
      
      ix_NA2[!ix_NA][sample(1:length(ix_NA2[!ix_NA]),nb_entries_add)] <- TRUE
      
      print("-------------------------")  
      print(paste0(round(sum(!ix_NA2)/(sum(ix_NA2)+sum(!ix_NA2)),digits = 2)*100,"% actual gaps"))
      print(paste0(Percent,"% gap_aim"))
      print(paste0(sum(rowSums(!ix_NA2)==ncol(ix_NA2))," rows without any values"))
      ...
      if(round(sum(ix_NA2)/(sum(ix_NA2)+sum(!ix_NA2)),digits = 2)==(100-Percent)/100){
        ...
        for(TDno in 1:2){
          ...
          ix_NA3 <- cbind(rep(TRUE,nrow=nrow(data_now)),ix_NA2)# take care for Observation ID
          dim(data_now)==dim(ix_NA3)
          data_nowNA <- data_now
          data_nowNA[!ix_NA3] <- NA
          print(sum(rowSums(!is.na(data_nowNA[,-1]))==0))
          
          if(ObsOrTD=="Obs_obs"){
            mtch <- match(data_nowNA$ObservationID,data_envelope$ObservationID)
            data_envelope$ObservationID[mtch]==data_nowNA$ObservationID
            ...
            data_envelope[mtch,mtch_traits] <- data_nowNA[,-grep(colnames(data_nowNA),pattern = "ObservationID")]
            colnames(data_envelope[mtch,match(trait_subset_names,colnames(data_envelope))] )
            data_save=data_envelope
          }
          if(ObsOrTD=="Obs_obs_TD"){data_save=data_nowNA}
          dim(data_save)
          ...
          unlink(file.path(origin,"_2021","data","_runs",paste0("Rep_",RepNum),t_choice,
                           paste0("p_",Percent),ObsOrTD,"data","traitInfo.csv"),recursive = TRUE)
