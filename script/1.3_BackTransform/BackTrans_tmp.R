...
out <- choices()
  ...
  if(Percent!=0|file.exists(pred_path)){ 
      ...
      traitInfo_obs_REzlog <- cbind(traitInfo_obs[,1],(log(traitInfo_obs[,-1])-zlog_trans$meanM)/zlog_trans$sdM)
      if(Percent!=0){ 
    ...
    #-------------------------------------------------------------------
      ...
      write.csv(traitInfo_obs_REzlog,file = file.path(origin,"_2021","data","_runs",paste0("Rep_",RepNum),t_choice,paste0("p_",Percent),ObsOrTD,"data","traitInfo_obs_REzlog.csv"))
