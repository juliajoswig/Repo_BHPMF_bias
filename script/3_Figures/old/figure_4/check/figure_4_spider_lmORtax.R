...
t=1
  ...
  library(fmsb)
  pdf(file=file.path(origin,"_2021","figures","Figure_4","Figure_4_Spider_zloglmOrtax.pdf"),width = 20,height=5)
  par(mar=c(2,2,2,2),mfrow=c(1,4))
  for(t in 1:6){
    ...
    radarchart(data_chart,pcol = colz_solid,axistype = 2,plty = 1,plwd = 4,pty = 32,axislabcol = "gray",
               title = "Many spec and gen",vlcex = 1.8,calcex = 1.5)
    ...
    names(data_now) <-gsub(names(data_now),pattern = "_obs",replacement = "") 
    names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
    data_now_obs_noTax <- as.data.frame(t(data_now))
    ...
    names(data_now) <-gsub(names(data_now),pattern = "_pred",replacement = "") 
    names(data_now) <-gsub(names(data_now),pattern = "_zlog",replacement = "") 
    data_now_pred_noTax <- as.data.frame(t(data_now))
    
    data_plot <- abs(rbind(data_now_obs_noTax,data_now_pred_noTax))
    data_chart <- rbind(rep(0,ncol(data_plot)) , rep(1,ncol(data_plot)) , data_plot)
    radarchart(data_chart,pcol = colz_solid,axistype = 2,plty = 1,plwd = 4,pty = 32,axislabcol = "gray",
               title = "Few spec and gen",vlcex = 1.8,calcex = 1.5)
    text(0,0,trait_names[t])
    ...
    data_chart <- rbind(rep(0,ncol(data_plot)) , rep(1,ncol(data_plot)) , data_plot)
    radarchart(data_chart,pcol = colz_solid,axistype = 2,plty = 1,plwd = 4,pty = 32,axislabcol = "gray",
               title = "High spec distance",vlcex = 1.8,calcex = 1.5,)
    text(0,0,trait_names[t])
    ...
    data_chart <- rbind(rep(0,ncol(data_plot)) , rep(1,ncol(data_plot)) , data_plot)
    radarchart(data_chart,pcol = colz_solid,axistype = 2,plty = 1,plwd = 4,pty = 32,axislabcol = "gray",
               title = "Low spec distance",vlcex = 1.8,calcex = 1.5,)
    text(0,0,trait_names[t])
