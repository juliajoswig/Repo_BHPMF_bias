...
dat_rmse <- dat_rmse[,grep(colnames(dat_rmse),pattern = "_tot")]
colnames(dat_rmse)
...
dat_rmse2 <- dat_rmse2[,grep(colnames(dat_rmse2),pattern = "_tot")]
colnames(dat_rmse2)
...
pdf(file=file.path(origin,"plots","Silhouettes","Sil_Gap_size.pdf"),width=12,height=6)
  ...
  plot(dat_plot2[dat_plot2[,1]!=(-1),c(1,i)],col=colz[i],pch=16,
       main=colnames(dat_plot)[i],xlab="Missingness [% gaps]",ylab="RMSE",xlim=c(0,72),ylim=c(0,1))
  ...
  lines(dat_plot2[dat_plot2[,1]!=(-1),c(1,i)],col="black",lwd=3.5)
  lines(dat_plot2[dat_plot2[,1]!=(-1),c(1,i)],col=colz,lwd=3)
