...
pdf(file=file.path(origin,"_2021","figures","Figure_1","Fig_S1_ValueWise.pdf"),width=10,height =8)
par(mfrow=c(1,1),mar=c(4,4,2,2))
par(mfrow=c(1,6),mar=c(7,6,2,1))

limmin <- c(-30,-42,-.7,-.15,-3000)
limmax <- c(30,42,.5,.2,3000)
...
pdf(file=file.path(origin,"_2021","figures","figure_1","Fig_S1_ValueWiseGAPSonly.pdf"),width=10,height =8)
par(mfrow=c(1,1),mar=c(4,4,2,2))
par(mfrow=c(1,7),mar=c(0,6,0,0))

plot(1:10,xaxt="n",yaxt="n",ylab="",xlab="",col="white",frame=FALSE)
text(2,5,srt=90,"Value",cex=3)
text(7,5,srt=90,"pred - observed",cex=3)
...
for(t in 1:length(trait_names)){
  ...
  bxpl=res$value_pred[ix_trait]- res$value_obs[ix_trait]
  bxplENV=resENV$value_pred[ix_trait]- resENV$value_obs[ix_trait]
  boxplot(cbind(bxpl,bxplENV),ylim=c(limmin[t],limmax[t]),ylab=paste0(trait_names[t]," [",units[t],"]"),width=1,las=2,col=colz[t],cex.axis=2,cex.lab=3,tick = FALSE,frame=FALSE)
  abline(h=0)
