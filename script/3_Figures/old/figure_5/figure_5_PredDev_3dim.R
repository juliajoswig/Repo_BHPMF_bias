...
png(filename=file.path(origin,"_2021","figures","Figure_5","fig_5_PredDev_spec.png"),width = 1500,height = 800)
...
#species
{
  ...
  # --------
  # prediction term for deviation
  # --------
  t=1
  summary(unlist(res_2[colnames(res_2)==paste0("CV_spec_pred_",trait_names[t])]-res_2[colnames(res_2)==paste0("CV_spec_obs_",trait_names[t])]))
  summary(unlist(0-res_2[colnames(res_2)==paste0("CV_spec_obs_",trait_names[t])]))
  CV_pred=unlist(res_2[colnames(res_2)==paste0("CV_spec_pred_",trait_names[t])])
  CV_obs=unlist(res_2[colnames(res_2)==paste0("CV_spec_obs_",trait_names[t])])
  dev_act=as.vector(unlist(res_2[colnames(res_2)==paste0("CV_spec_pred_",trait_names[t])]-res_2[colnames(res_2)==paste0("CV_spec_obs_",trait_names[t])]))
  dev_act[dev_act==0] <- 0.0000001
  dev_pred=as.vector(unlist(0-res_2[colnames(res_2)==paste0("CV_spec_obs_",trait_names[t])]))
  dev_pred[dev_pred==0] <- 0.0000001
  m=# actual deviation
    dev_act/
    # predicted deviation based on CV = 0
    dev_pred
  
  plot(# actual deviation
    dev_act,
    # predicted deviation based on CV = 0
    dev_pred)
  abline(0,1)
  ...
  summary(m)
  hist(m)
  m1 <- mean(m,na.rm = TRUE)        
  
  # test if it works  colz=rep(NA,nrow(CV_obs))
  colz=rep(NA,length(CV_obs))
  colz[res_2$nb_spec==1] <- rainbow(8)[1]
  colz[res_2$nb_spec==2] <- rainbow(8)[2]
  colz[res_2$nb_spec>2] <- rainbow(8)[3]
  colz[res_2$nb_spec>5] <- rainbow(8)[4]
  colz[res_2$nb_spec>8] <- rainbow(8)[5]
  colz[res_2$nb_spec>10] <- rainbow(8)[6]
  colz[res_2$nb_spec>15] <- rainbow(8)[7]
  colz[res_2$nb_spec>25] <- rainbow(8)[8]
  #plot(dev_pred*m1,dev_act,col=colz)
  abline(0,1)
  # Ok these residuals will be dealt with by checking the nb of samples
  
  
  y1 <- unlist(0-res[colnames(res)==paste0("CV_spec_obs_",trait_names[t])]*m1)
  plot(abs(y1),unlist(res[colnames(res)=="nb_spec"]))
  
  plot(cbind(d1,res[colnames(res)==paste0("CV_spec_pred_",trait_names[t])]-res[colnames(res)==paste0("CV_spec_obs_",trait_names[t])]))
  abline(0,1)
  
  plot(cbind(d1/res[colnames(res)==paste0("CV_spec_pred_",trait_names[t])]-res[colnames(res)==paste0("CV_spec_obs_",trait_names[t])],
             res[colnames(res)==paste0("CV_spec_pred_",trait_names[t])]-res[colnames(res)==paste0("CV_spec_obs_",trait_names[t])]))
         
  cor(cbind(d1/res[colnames(res)==paste0("CV_spec_pred_",trait_names[t])]-res[colnames(res)==paste0("CV_spec_obs_",trait_names[t])],
            res[colnames(res)==paste0("CV_spec_pred_",trait_names[t])]-res[colnames(res)==paste0("CV_spec_obs_",trait_names[t])]))
  
  #try with another trait:  
  t=2
  plot(cbind(d1/res[colnames(res)==paste0("CV_spec_pred_",trait_names[t])]-res[colnames(res)==paste0("CV_spec_obs_",trait_names[t])],
             res[colnames(res)==paste0("CV_spec_pred_",trait_names[t])]-res[colnames(res)==paste0("CV_spec_obs_",trait_names[t])]))

  
  colz=rep(NA,nrow(CV_obs))
  colz2=rep(NA,nrow(CV_obs))
  colz2[res_2$nb_spec>3] <- rainbow(8)[3]
  ...
  colz[res_2$nb_spec>15] <- rainbow(8)[7]
  colz[res_2$nb_spec>25] <- rainbow(8)[8]
  plot(CV_obs,CV_pred,col=colz,pch=16,cex=.5)
  plot(0-CV_obs,CV_pred-CV_obs,col=colz,pch=16,cex=.5)
  points(0-CV_obs,CV_pred-CV_obs,col=colz2,pch=16,cex=2)
  abline(0,1)
  
  dev.off()
  par(mfrow=c(1,1))
  dat_plot = CV_pred-CV_obs
  ymin=-40
  ymax=20
  boxplot((dat_plot),(dat_plot)[res_2$nb_spec==2],
          dat_plot[res_2$nb_spec>2&res_2$nb_spec<5],
          dat_plot[res_2$nb_spec>5&res_2$nb_spec<8],
          dat_plot[res_2$nb_spec>8&res_2$nb_spec<12],
          dat_plot[res_2$nb_spec>12&res_2$nb_spec<17],
          dat_plot[res_2$nb_spec>17&res_2$nb_spec<25],
          dat_plot[res_2$nb_spec>25],col=rainbow(8),ylim=c(ymin,ymax))
  abline(h=median(dat_plot,na.rm=TRUE))
  ...
  data$x=dev_act
  data$y=dev_pred
  data$z=res_2$nb_spec
  
  data_df = as.data.frame(cbind(CV_obs,CV_pred,res_2$nb_spec))
  colnames(data_df) <- c("x","y","z")
  data_df <- data_df[data_df$z!=0,]
  
  data_df$cat <- rep(NA,nrow(data_df))
  data_df$cat[which(data_df$z == 1)] <- 1
  data_df$cat[which(data_df$z == 2)] <- 2
  data_df$cat[which(data_df$z > 2)] <- 3
  data_df$cat[which(data_df$z > 5)] <- 5
  data_df$cat[which(data_df$z > 15)] <- 6
  data_df$cat <- as.factor(data_df$cat)
  
  fig <- plot_ly(data_df, x = ~x, y = ~y, z = ~z, color = ~cat, colors = rainbow(6))
  fig <- fig %>% add_markers()
  fig <- fig %>% layout(scene = list(xaxis = list(title = 'CV pred'),
                                     yaxis = list(title = 'CV obs'),
                                     zaxis = list(title = 'sample number')))
  
  fig
  
  
  plot_ly(data_df,x=x,y=y,z=z, type="surface")  
