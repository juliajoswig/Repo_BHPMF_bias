...
res <- read.csv(file=file.path(origin,"_2021","data","analyses","Point_wise",RepNum,t_choice,ObsOrTD,Percent,paste0("res_2021.csv")))
  ...
  w=1
    ...
    colnames(bxpl_sp_op) <- rep(c("obs","pred"),5)
    colnames(bxpl_gen_op) <- rep(c("obs","pred"),5)
    colnames(bxpl_fam_op) <- rep(c("obs","pred"),5)
    colnames(bxpl_clad_op) <- rep(c("obs","pred"),5)
    colnames(bxpl_GF_op) <- rep(c("obs","pred"),5)
    colnames(bxpl_PFT_op) <- rep(c("obs","pred"),5)
    
    colz=c("#b2182b","#b2182b","#ef8a62","#ef8a62","#fddbc7","#fddbc7","#f7f7f7","#f7f7f7",
           "#d1e5f0","#d1e5f0","#67a9cf","#67a9cf","#2166ac","#2166ac")
  
    pdf(file=file.path(origin,"_2021","figures","Figure_2",paste0("Figure_S2_Dist",missingness[m],".pdf")),width=20,height=8)
    {
      par(mar=c(4,0,4,0))
      layout(matrix(c(1:6,1:6,1:6,1:6,
                      7,rep(8,5)), 
      ...
      for(t in 1:5){
        ...
        yBot_now=apply(bxpl[,seq(from=1,to = 10,by = 2)],2,FUN = median,na.rm = TRUE)
        i=1
