...
res_now=res_sing
  par(mfrow=c(1,1),mar=c(10,6,2,2))
    ...
    bxpl_sp=matrix(NA,nrow=sum(ix_trait&!duplicated(res_now$Species)[ix_trait]),ncol=2)
    bxpl_gen=matrix(NA,nrow=sum(ix_trait&!duplicated(res_now$Genus)[ix_trait]),ncol=1)#rep(NA,sum(ix_trait))
    bxpl_fam=matrix(NA,nrow=sum(ix_trait&!duplicated(res_now$Family)[ix_trait]),ncol=1)
    bxpl_clad=matrix(NA,nrow=sum(ix_trait&!duplicated(res_now$Clade)[ix_trait]),ncol=1)
    bxpl_GF=matrix(NA,nrow=sum(ix_trait&!duplicated(res_now$GF)[ix_trait]),ncol=1)
    bxpl_PFT=matrix(NA,nrow=sum(ix_trait&!duplicated(res_now$PFT)[ix_trait]),ncol=1)
    ...
    pdf(file=file.path(origin,"_2021","figures","Figure_2",paste0("Figure_2_Sil_ENV.pdf")),width=17,height=17)
