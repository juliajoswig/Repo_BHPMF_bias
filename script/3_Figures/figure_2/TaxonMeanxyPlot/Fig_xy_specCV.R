...
CV.fun <- function(input){
  if(sum(is.na(input))==length(input)){output=NA
  }else{
    output=sd(input[!is.na(input)])/mean(input[!is.na(input)])
  }
  return(output)
}  
  
# aggregate TD to species CV.fun
TD_sp <- aggregate(TD,by=list(TD_tax$Species),FUN = CV.fun)
TDtd_sp <- aggregate(TDtd,by=list(TD_tax$Species),FUN = CV.fun)
TDenv_sp <- aggregate(TDenv,by=list(TD_tax$Species),FUN = CV.fun)
...
#                            rgb(255,255,191,maxColorValue = 255,alpha=255/2),
...
if(t_choice=="data"){
  pdf(file=file.path(origin,"_2021","figures","figure_2","Fig_xy_speciesCV.pdf"),width=10,height=5)
}
if(t_choice=="data_2"){
  pdf(file=file.path(origin,"_2021","figures","figure_2","Fig_Sxy_speciesCV.pdf"),width=10,height=5)
}
{
  ...
  plot(TD_sp[,t],TDtd_sp[,t],col=colz,pch=16,xlim=c(0,1),ylim=c(0,1),
       xlab="TD, species CV",cex=.6,
       ylab="TDtd, species CV")
  head(plot_data)
  ...
  text(x = .5,y = .9, labels = paste0("Pearson cor=",round(cor(plot_data[complete.cases(plot_data),])[2,1],digits=2)))
  abline(0,1)
  ...
  plot(TD_sp[,t],TDenv_sp[,t],col=colz,pch=16,xlim=c(0,1),ylim=c(0,1),
       xlab="TD, species CV",cex=.6,
       ylab="TDenv, species CV")
  for(t in 3:ncol(TD_sp)){
