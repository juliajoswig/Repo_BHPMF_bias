...
out <- choices(originData)
  ...
  sd.fun <- function(input){
    if(sum(!is.na(input))==0){return(0)}else{
      return(sd(input[!is.na(input)]))}
  }
...
#    file.path(originData,"analyes","Point_wise","res.csv")
    ...
    #load data
  {
    ...
    #_________________________________________________
    # aggregate TD to taxon mean ZLOG
    #-------------------------------------------------
...
if(tx!=1){  
    ...
    #-------------------------------------------------
    TD_meantx <- aggregate(TD,by=list(TD_tax[,tx]),FUN = mean.fun)
    TD_sparse_meantx <- aggregate(TD_sparse,by=list(TD_tax[,tx]),FUN = mean.fun)
    TDtd_meantx <- aggregate(TDtd,by=list(TD_tax[,tx]),FUN = mean.fun)
    TDenv_meantx <- aggregate(TDenv,by=list(TD_tax[,tx]),FUN = mean.fun)
    
    TD_meantx <- TD_meantx[,-which(colnames(TD_meantx)=="Group.1")]
    TD_sparse_meantx <- TD_sparse_meantx[,-which(colnames(TD_sparse_meantx)=="Group.1")]
    TDtd_meantx <- TDtd_meantx[,-which(colnames(TDtd_meantx)=="Group.1")]
    TDenv_meantx <- TDenv_meantx[,-which(colnames(TDenv_meantx)=="Group.1")]
    #_________________________________________________
    # aggregate TD to taxon sd
    #-------------------------------------------------
    TD_sdtx <- aggregate(TD,by=list(TD_tax[,tx]),FUN = sd.fun)
    TD_sparse_sdtx <- aggregate(TD_sparse,by=list(TD_tax[,tx]),FUN = sd.fun)
    TDtd_sdtx <- aggregate(TDtd,by=list(TD_tax[,tx]),FUN = sd.fun)
    TDenv_sdtx <- aggregate(TDenv,by=list(TD_tax[,tx]),FUN = sd.fun)
    
    TD_sdtx <- TD_sdtx[,-which(colnames(TD_sdtx)=="Group.1")]
    TD_sparse_sdtx <- TD_sparse_sdtx[,-which(colnames(TD_sparse_sdtx)=="Group.1")]
    TDtd_sdtx <- TDtd_sdtx[,-which(colnames(TDtd_sdtx)=="Group.1")]
    TDenv_sdtx <- TDenv_sdtx[,-which(colnames(TDenv_sdtx)=="Group.1")]
...
#    try(dev.off())
    pdf(file=file.path(origin,"figures","Figure_2","correls",paste0(txs[tx],"_corsCV_",t_choice,".pdf")),width=8,height=6)
    layout(mat=matrix(c(1,1,1,2,2,2,
               3,4,5,6,7,8),nrow=2,byrow=TRUE))
    par(mar=c(4,8,1,1))
    {   
      #OBS
      t=1 
      dat_ploto = cbind((TD_sdtx/TD_meantx),(TDtd_sdtx/TDtd_meantx))[seq(from = t,to = ncol(TD)*2,by = ncol(TD))]
      ...
      dat_plotot = cbind(unlist(TD_sdtx/TD_meantx),unlist(TDtd_sdtx/TDtd_meantx))
      dat_plotot <- dat_plotot[complete.cases(dat_plotot),]
      coro <- cor(dat_plotot)
      mtext(paste0("Pearson corr. = ",round(coro[1,2],digits=2),"  "),cex=1,
            side = 3, adj = 1, line = - 3)
      legend("topleft",trait_names, pch=16, title= "Traits", inset = .05, cex=1,
             col=colz[1:length(trait_names)])
      
      #--------------------------------------------------------------    
      #Ext
      t=1
      dat_plote= cbind((TD_sdtx/TD_meantx),
                       (TDenv_sdtx/TDenv_meantx))[seq(from = t,to = ncol(TD)*2,by = ncol(TD))]
      ...
      for(t in 1:ncol(TD)){
        dat_plote= cbind(      (TD_sdtx/TD_meantx),   (TDenv_sdtx/TDenv_meantx))[seq(from = t,to = ncol(TD)*2,by = ncol(TD))]
        
        #Ext
        points(dat_plote,cex.lab=2,col=colz[t],pch=16)
      }   
      abline(0,1,col="gray",lty=2,lwd=2)
      dat_plotet= cbind(unlist(TD_sdtx/TD_meantx),unlist(TDenv_sdtx/TDenv_meantx))
      core <- cor(dat_plotet[complete.cases(dat_plotet),])
      mtext(paste0("Pearson corr. = ",round(core[1,2],digits=2),"  "),cex=1,
            side = 3, adj = 1, line = - 3)
      legend("topleft",trait_names, pch=16, title= "Traits", inset = .05, cex=1,
             col=colz[1:length(trait_names)])
    ...
    par(mar=c(5,2,3,1))
    for(t in 1:ncol(TD)){
      dat_now=cbind((TD_sdtx/TD_meantx),(TDtd_sdtx/TDtd_meantx),(TDtd_sdtx/TDenv_meantx))[,seq(from = t,to = ncol(TD)*3,by = ncol(TD))]
      colnames(dat_now) <- c("OBS","IMPobs","IMPobsExt")
    boxplot(dat_now,ylim=c(0,1),las=2,
            col=colz[t],main=colnames(TD)[t],cex.main=1.8,cex.lab=.8)
    }
...
#--------------------------------------------------------------    
    ...
    if(tx==2){
    for(t in 1:ncol(TD)){
      pdf(file=file.path(origin,"figures","Figure_2","correls",paste0(txs[tx],colnames(TD)[t],"_cors_",t_choice,".pdf")),width=12,height=6)
      par(mfrow=c(1,2),mar=c(4,6,1,1))
      dat_ploto= cbind(      (TD_tx),   (TDtd_tx))[seq(from = t,to = ncol(TD)*2,by = ncol(TD))]
      dat_plote= cbind(      (TD_tx),   (TDenv_tx))[seq(from = t,to = ncol(TD)*2,by = ncol(TD))]
      ...
      colnames(dat_ploto)=  c("OBS","IMPobs")
      coro <- cor(dat_ploto)
      plot(dat_ploto,cex.lab=2,col=colz[t],pch=16,xlim=c(-4,4),ylim=c(-4,4))
      abline(0,1,col="gray",lty=2,lwd=2)
      mtext(paste0("Pearson corr. coeff. = ",round(coro[1,2],digits=2),"  "),cex=2,
            side = 1, adj = 1, line = - 3)
      ...
      colnames(dat_plote)=  c("OBS","IMPobsExt")
      core <- cor(dat_plote)
      plot(dat_plote,cex.lab=2,col=colz[t],pch=16,xlim=c(-4,4),ylim=c(-4,4))
      abline(0,1,col="gray",lty=2,lwd=2)
...
pdf(file=file.path(origin,"figures","Figure_2","correls",paste0(txs[tx],"_corsCV_",t_choice,".pdf")),width=12,height=6,pointsize = 9)
...
{   
  ...
  abline(0,1,col="gray",lty=2,lwd=2)
  dat_plotot= cbind(unlist(TD_sdtx/TD_meantx),unlist(TDtd_sdtx/TDtd_meantx))
  coro <- cor(dat_plotot[complete.cases(dat_plotot),])
  mtext(paste0("Pearson corr. coeff. = ",round(coro[1,2],digits=2),"  "),cex=2,
        side = 1, adj = 1, line = - 3)
  legend("topleft",trait_names, pch=16, title= "Traits", inset = .05, cex=1.4,
         col=colz[1:length(trait_names)])
