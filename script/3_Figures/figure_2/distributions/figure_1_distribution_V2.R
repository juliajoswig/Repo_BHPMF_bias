...

{
  ...
  for(t in 1:length(trait_names)){
    ...
    if(!defense_prep){
      if(t==1&t_choice=="data"){
        legend(11,.15,legend = c("OBS","TRY17","IMP_obs","IMP_obsExt"),col = colz[c(6,7,2,1)],
               lwd=4,lty=1,cex=1.9,bty = "n")
      }
      if(t==1&t_choice=="data_2"){
        legend(11,.15,legend = c("OBS2","TRY17","IMP2_obs","IMP2_obsExt"),col = colz[c(6,7,2,1)],
               lwd=4,lty=1,cex=1.9,bty = "n")
      }
    }
    if(defense_prep){
