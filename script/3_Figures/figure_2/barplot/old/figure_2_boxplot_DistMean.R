    ...
    pdf(file=file.path(origin,"_2021","figures","Figure_2",paste0("Figure_2_Dist_Env.pdf")),width=20,height=8)
    {
      ...
      for(t in 1:6){
       ...
       # axis(side = 1,line = 3, at = seq(from=1.5,to = 12,by = 2),las=2,labels=trait_names,tick = FALSE,cex.axis=2)
        ...
        points(c(seq(2,12,2)),apply(bxpl[,seq(3,ncol(bxpl),3)],2,FUN=median,na.rm = TRUE),pch=16,col="black",cex=3)
        points(c(seq(2,12,2)),apply(bxpl[,seq(3,ncol(bxpl),3)],2,FUN=median,na.rm = TRUE),pch=16,col="white",cex=2)
        points(c(seq(2,12,2)),apply(bxpl[,seq(3,ncol(bxpl),3)],2,FUN=median,na.rm = TRUE),pch=8,col="white",cex=3)
      }
