...
list.files(file.path(origin,"_2021","figures","Figure_1"))
  ...
  par(mar=c(7,7,1,1))
      ...
      # Total data TDtd
      {
      plot(plot_TD2td[,c(1,i)],col=colz[i],pch=16,cex=.1,cex.lab=3,cex.axis=3,xaxt="n",yaxt="n",
      ...
      for(i in 1:length(trait_names)){
        print(trait_names[i])
        lines(plot_TD2td[,1],plot_TD2td[,colnames(plot_TD2td)==paste0("RMSE_zlog_",trait_names[i])],col=colz[i],lwd=3)
      }  
      ...
      # Total TDext
      {
        i=2
        # Total data TDext
        plot(plot_TD2ext[,c(1,i)],col=colz[i],pch=16,cex=.1,cex.lab=3,cex.axis=3,xaxt="n",yaxt="n",
             xlab="",ylab="",xlim=c(0,80),ylim=c(0,.6),frame=FALSE)
        axis(2,line = 3,at = c(0,.3,.6),labels = c("","RMSE",""),tick = FALSE,cex.axis=2)
        axis(2,line = 1,at = seq(0,.6,.1),labels = seq(0,.6,.1),cex.axis=2)
        ...
        for(i in 1:length(trait_names)){
          print(trait_names[i])
          lines(plot_TD2ext[,1],plot_TD2ext[,colnames(plot_TD2ext)==paste0("RMSE_zlog_",trait_names[i])],col=colz[i],lwd=3)
        }  
      ...
      #Gaps TDtd
      {
        ...
        for(i in 1:length(trait_names)){
          print(trait_names[i])
          lines(plot_TD2td[,1],plot_TD2td[,colnames(plot_TD2td)==paste0("RMSE_gap_zlog_",trait_names[i])],col=colz[i],
                lwd=3,lty=1)
        }  
        
        i=2
        lines(plot_TD2td[,1],plot_TD2td[,colnames(plot_TD2td)=="RMSE_gap_zlog_Total"],col="black",lwd=3,lty=1)
...
#        abline(h=seq(-1.2,1.2,by=.1),col="gray",lty=2)
        ...
        for(i in 1:length(trait_names)){
          print(trait_names[i])
          lines(plot_TD2ext[,1],plot_TD2ext[,colnames(plot_TD2ext)==paste0("RMSE_gap_zlog_",trait_names[i])],col=colz[i],
                lwd=3,lty=1)
        }  
        
        i=2
        lines(plot_TD2ext[,1],plot_TD2ext[,colnames(plot_TD2ext)=="RMSE_gap_zlog_Total"],col="black",lwd=3,lty=1)
